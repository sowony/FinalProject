...
function ruleListOpen(){
	...
	const ruleListDiv = addObject(null, 'div', 'ruleListDiv', false, (o)=>{
		...
		dashgrade.forEach(rule=>{
			...
			addObject(o, 'p', 'ruleItem', true, (o)=>{
				...
				o.innerHTML = `<span>${rule.dgalias}</span><span class="dggrade" data-dggrade="${rule.dggrade}">${rule.dggrade}</span>`;
				o.addEventListener('click', (e)=>{
					
					const widgetRuleSetting = document.querySelector('.widgetRuleSetting');
					
					const wrmin = widgetRuleSetting.querySelector('input[name=wrmin]');
					const wrmax = widgetRuleSetting.querySelector('input[name=wrmax]');
					...
					const select = o.parentNode.querySelectorAll('.select');
					if(select && select.length === 2){
						const ruleList = document.querySelector('.ruleList');
						
						const num0 = select[0].querySelector('.dggrade').dataset.dggrade
						const num1 = select[1].querySelector('.dggrade').dataset.dggrade
	...
	boxFun('현재 보드의 모든 룰 리스트',false, [ruleListDiv], false, 'ruleList', null, true);
...
function widgetAdd(){
	...
	const widgetRuleSetting = addObject(null, 'div', 'widgetRuleSetting', false, (o)=>{
		...
		const widgetRuleIndividualBox = addObject(null, 'div', 'widgetRuleIndividualBox', false, (o)=>{
			
			o.innerHTML = `
			<div>
				<p style="display:block;">맴버 조회<a href="#" style="font-size:8pt;" onclick="memberListOpen">모든 맴버 보기</a></p>
				<input type="text" name="mnick" style="margin: 5px 0;width: 70%;"/>
				<input type="button" style="width:max-content;height:34px;" class="grayBtn" value="유저 조회"/>
			</div>
		...
		// 그룹 박스
		const widgetRuleGroupBox = addObject(null, 'div', 'widgetRuleGroupBox', false, (o)=>{
			...
			o.innerHTML = `
				<div>
					<p style="display:block;">최소 & 최대 접근 등급 <a href="#" style="font-size: 8pt;" onclick="ruleListOpen();">모든 권한 보기</a></p>
			...
			submitBtn.addEventListener('click',(e)=>{
				...
				if(wrcategory && wrrwd && wrmin.value && wrmax.value){
					const widgetRuleList = e.target.parentNode.querySelector('.widgetRuleList');
	...
	const widgetAddNextBtn = addObject(null, 'input', ['widgetAddNextBtn', 'grayBtn'], false, (o)=>{
		...
		o.addEventListener('click', (e)=>{
			...
			boxFun(null, false, [widgetRuleSetting, widgetAddCloseBtn, widgetAddPrevBtn], true, 'widgetAddBox2', (o)=>{
