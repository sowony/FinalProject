	...
	FB.login((res)=>{
		...
		if(res.status === 'connected'){
			FB.api('/me',(data)=>{
        		...
        		boxFun(`${data.name}님 사이트에  오신 것을  환영합니다!<br> <span style="color:#3d3d3d; font-size:9pt;">사이트 이용에 있어서 사용하실</span><br><span style="color:#3d3d3d; font-size:9pt;">닉네임과 성함 및 이메일을 작성해주세요.</span>`, true, [profileImg, platformSignUpBox], false, 'platformSignUpBox', (o, child, selector)=>{
           					...
           					input.addEventListener('click', ()=>{
           						...
           						if(signUpCheck) {
           							if(mailCheck) {
           								const memberInfo = {
           										'mid' : data.id,
           										'mpw' : data.id + window.btoa(data.name) + ((Math.random() * 999999) + 100000),
           										'mname' : data.name,
